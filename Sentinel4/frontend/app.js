...
function calculateClientRisk(mId) {
    ...
    // Align risk calculation with individual value color thresholds
    // Only increase risk when values would actually show red
    
    // Temp: red at >85°C (critical) or >100°C (severe critical)
    if (metrics.temperature_C > 85) risk += 0.4;
    
    // Vib: red at >3 mm/s (critical) or >5 mm/s (severe critical)
    if (metrics.vibration_mm_s > 3) risk += 0.4;
    
    // Load: red at >18A (critical) or >22A (severe critical)
    if (metrics.current_A > 18) risk += 0.2;
    
    // RPM: red when outside ±20% warning band of baseline
    const rpmWarnHigh = base.rpm * 1.2;
    const rpmWarnLow = base.rpm * 0.8;
    if (metrics.rpm > rpmWarnHigh || metrics.rpm < rpmWarnLow) risk += 0.2;
...
function startLocalSimulation() {
    ...
    // Matches HM server update frequency: 1 reading per second
    setInterval(() => {
        machines.forEach(m => {
            if (machineData[m].lastUpdated && (new Date() - machineData[m].lastUpdated < 1000)) return; // Don't override fresh SSE data
...
// HM-style threshold coloring for overview value tiles only.
function getOverviewTempClass(value) {
    if (value > 100) return 'text-error';
    if (value > 85) return 'text-error';
    return 'text-on-background';
}

function getOverviewVibClass(value) {
    if (value > 5) return 'text-error';
    if (value > 3) return 'text-error';
    return 'text-on-background';
}

function getOverviewLoadClass(value) {
    if (value > 22) return 'text-error';
    if (value > 18) return 'text-error';
    return 'text-on-background';
}

function getOverviewRpmClass(machineId, value) {
    const base = baseValues[machineId];
    if (!base || !base.rpm) return 'text-on-background';

    const warnHigh = base.rpm * 1.2;
    const warnLow = base.rpm * 0.8;
    const critHigh = base.rpm * 1.35;
    const critLow = base.rpm * 0.65;

    if (value > critHigh || value < critLow) return 'text-error';
    if (value > warnHigh || value < warnLow) return 'text-error';
    return 'text-on-background';
}
        ...
        card.innerHTML = `
            ...
            <div class="grid grid-cols-2 gap-4 mb-6">
                <div class="bg-surface-container-lowest p-3">
                    ...
                    <p class="font-mono text-lg ${getOverviewTempClass(Number(data.metrics.temperature_C || 0))} font-bold">${data.metrics.temperature_C?.toFixed(1) || '--'}°C</p>
                </div>
                <div class="bg-surface-container-lowest p-3">
                    <p class="text-[9px] font-headline tracking-widest text-outline mb-1 uppercase">VIB</p>
                    <p class="font-mono text-lg ${getOverviewVibClass(Number(data.metrics.vibration_mm_s || 0))} font-bold">${data.metrics.vibration_mm_s?.toFixed(2) || '--'}</p>
                </div>
                <div class="bg-surface-container-lowest p-3">
                    <p class="text-[9px] font-headline tracking-widest text-outline mb-1 uppercase">RPM</p>
                    <p class="font-mono text-lg ${getOverviewRpmClass(m, Number(data.metrics.rpm || 0))} font-bold">${data.metrics.rpm?.toFixed(0) || '--'}</p>
                </div>
                <div class="bg-surface-container-lowest p-3">
                    <p class="text-[9px] font-headline tracking-widest text-outline mb-1 uppercase">LOAD</p>
                    <p class="font-mono text-lg ${getOverviewLoadClass(Number(data.metrics.current_A || 0))} font-bold">${data.metrics.current_A?.toFixed(1) || '--'}A</p>
                </div>
            </div>
